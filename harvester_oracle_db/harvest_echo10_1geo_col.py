...
OUTPUT_ROOT_DIR = './data_echo_lpcloud_plain' # Root directory of saved output, which can have collection sub-directories if desired (see harvest_granules)
...
def parse_granule_geometries(granule_xml):
    ...
    if point_lon is not None and point_lat is not None:
        ...
        geometries.append((granule_id, start_time, end_time, Point(lon, lat)))
    ...
    if line_points:
        ...
        geometries.append((granule_id, start_time, end_time, LineString(line_coords)))

    # Parse BoundingRectangle (as a Polygon)
    ...
    if west is not None and east is not None and north is not None and south is not None:
        ...
        geometries.append((granule_id, start_time, end_time, Polygon(coords)))
    ...
    if gpolygons:
        ...
        geometries.append((granule_id, start_time, end_time, Polygon(gpoly_coords)))
...
def process_echo10_granules(granules_xml_list):
    ...
    try:
        ...
        return gpd.GeoDataFrame(pd.DataFrame(all_geometries, columns=['GranuleID', 'StartTime', 'EndTime', 'geometry']))
...
def process_echo10_batch(granules):
    ...
    try:
        ...
        for granule in granules:
            metadata = gzip.decompress(granule[2]).decode("utf-8")
            all_geometries.extend(parse_granule_geometries(metadata))
    
        return gpd.GeoDataFrame(all_geometries, columns=['GranuleID', 'StartTime', 'EndTime', 'geometry'])
